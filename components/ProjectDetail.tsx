...
import { Project, Task, TaskStatus, Priority, User, Attachment, ProjectMember } from '../types';
  ...
  return (
    <div className="relative inline-block" ref={containerRef}>
      <button
        ...
        className={className || `flex items-center gap-2 px-3 py-2 rounded-lg border transition-all ${
...
const TaskItem: React.FC<{
  ...
  return (
    <div 
      ...
      className={`relative group bg-white p-4 rounded-xl border transition-all flex flex-col sm:flex-row sm:items-start gap-4 ${
        isDone ? 'border-slate-100 bg-slate-50/50' : 'border-slate-200 hover:border-indigo-300 shadow-sm'
          ...
          <div className="flex-1">
            {isEditingTitle && canEdit ? (
              ...
              <h3 
                ...
                className={`text-lg font-medium transition-all leading-tight ${
            ...
            <form onSubmit={handleSaveProjectEdit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                    <UserSelect 
                        users={users.filter(u => !editProjectTeam.some(m => m.userId === u.id))}
      ...
      <div className={`flex-1 flex flex-col h-full transition-all duration-300`}>
            ...
            <div>
               <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider border ${
                      userRole === 'Owner' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' :
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8">
          <div className="max-w-4xl mx-auto space-y-8">
            ...
            {uncategorizedTasks.length === 0 && (
               <div 
                  className={`pt-4 mt-4 border-t border-dashed border-slate-200 transition-all rounded-xl p-4 text-center text-sm ${
                      dragOverMilestoneId === 'uncategorized' ? 'bg-indigo-50 ring-2 ring-indigo-200 text-indigo-600 border-indigo-300' : 
              ...
              <div className="flex flex-col gap-4 pt-4 border-t border-slate-100">
                ...
                <div className="flex items-center justify-between relative">
                  ...
                  <button 
                    ...
                    disabled={!canEdit}
                    className={`flex items-center gap-2 px-2 py-1 rounded-lg transition-colors border border-transparent ${
                      canEdit ? 'hover:bg-slate-100 hover:border-slate-200 cursor-pointer' : 'cursor-not-allowed opacity-70'
                    }`}
                  >
                ...
                <div className="flex items-center justify-between relative">
                    ...
                    <div className="relative">
                        <button 
                          ...
                          disabled={!canEdit}
                          className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${getPriorityColor(activeTask.priority)} ${
                            canEdit ? 'hover:brightness-95 cursor-pointer' : 'cursor-not-allowed opacity-80'
                          }`}
                        >
                          {activeTask.priority}
                        </button>
